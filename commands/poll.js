...
module.exports = {
    ...
    run: async (bot, message, args, messageArray, bargs, Discord, color, used, getUserFromMention) => {
        ...
        if(!messageArray[1]) return message.channel.send(`Provide Poll Name e.g. !poll.js The Champions`);
        let pollname = message.content.slice(9);
        if(userCreatedPoll.has(message.author.id)) return message.reply(`You have a poll going on`)
        const results = message.member.guild.channels.find(ch => ch.name === 'poll-results');
        if(!results) message.channel.send(`**You Need to create a channel named poll-results**`)
       ...
       let collector = message.channel.createMessageCollector(filter, {maxMatches: 3});
       let pollOptions = await getPollOptions(collector);
       if(pollOptions.length < 2) return message.reply(`Must Contain at least 2 Options Max: 3`)
       if(pollOptions.length > 3) return message.reply(`Options must be either 2 or 3(max)`)
       let embed = new Discord.RichEmbed();
       embed.setTitle(`Poll \n`)
       embed.setDescription(`**Poll Name:** ${messageArray[1]}`)
       embed.setDescription(pollOptions.join(`\n \n`))
       embed.addField(`Info:`, `React ${agr} to Start Poll or React ${agre} to Cancel Poll`)
       ...
       if(reaction.emoji.name === agr) {
           ...
           results.send(`\`\`\`A New Poll has been Created by ${message.author.id}  \n Poll Name: ${pollname}!\`\`\``)
           await delay(3000)
           message.channel.send(`\`\`\`Vote Now!!\`\`\``);
           let userVotes = new Map();
           let pollTally = new Discord.Collection(pollOptions.map(o => [o, 0]))
           let pollFilter = m => !m.bot;
           let voteCollector = message.channel.createMessageCollector(pollFilter, {time: 3600020});
           ...
           entries.forEach(entry => desc += `\n \n` + entry[0].toUpperCase() + ` recieved ` + entry[1] + ` Vote(s) \n`)
           results.send(embed2.setDescription(desc)) 
           if(winners.length === 1) {
               results.send(embed2.setDescription(`${winners[0].toUpperCase()} is the Winner 🏆 `).setColor(color))
           } else {
               results.send(embed2.setDescription(`Its a Draw ${agree}`).setColor(`YELLOW`))
           }
       ...
       function processPollResults(voteCollector, pollOptions, userVotes, pollTally) {
           return new Promise((resolve,reject) => {
               ...
               voteCollector.on(`end`, collected => {
                   ...
                   results.send(embed3.setDescription(`Collected ` + collected.size + ` Votes.`).setColor(`RED`).setTitle(`**PollName: ${pollname}**`))
                   resolve(collected);
               })
           })
       }
