module.exports = {
  ...
  run: async (
    ...
    if (!rMember)
      return message.channel.send(
        "Please provide a user to add a role too. !givrol.js [user] [role] [reason]"
      );
    let role =
      message.guild.roles.find((r) => r.name == messageArray[2]) ||
      message.guild.roles.find((r) => r.id == messageArray[2]) ||
      message.mentions.roles.first();
    if (!role)
      return message.channel.send(
        "Please provide a role to add to said user, Give Role Id or Mention Role. !givrol.js [user] [role] [reason]"
      );
    let reason = args.slice(2).join(" ");
    if (!reason)
      return message.channel.send(
        "Please provide a reason !givrol.js [user] [role] [reason]"
      );
    ...
    if (rMember.roles.has(role.id)) {
      ...
      message.channel.send(
        `The role, ${
          role.name
        }, has been added to ${message.mentions.members.first()}.`
      );
    }
